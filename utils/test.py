...
def test(model, tokenizer, test_df, device, prompt_id=1, essay_id=100):
    ...
    print(score_essay(topic, right_essay, tokenizer, model, device))
    
    print("++++++++++++++++++++++++++++")
    print("CASE_1: Less than 20 words")
    edited_essay = ' '.join(right_essay.split()[:19])
    print(score_essay(topic, edited_essay, tokenizer, model, device))
    ...
    print(score_essay(topic, swapped_essay, tokenizer, model, device))
    ...
    print(score_essay(topic, re.sub(punctuation_pattern, '', right_essay), tokenizer, model, device))
    ...
    print(score_essay(topic, random_text, tokenizer, model, device))
